...
Robby the robber. We followed him to a secret warehouse, where we assume to find all the stolen stuff. 
...
def get_pins(observed:str):
    allValues = []
    values = [int(i) for i in str(observed)]
    for value in values:
        match value:
            case 1:
                posValues = [1,2,4]
            ...
            case 0:
                posValues = [0,8]
        posValues =  map(str, posValues)
        allValues.append(posValues)
    output = map("".join, list(itertools.product(*allValues)))
    return list(output)